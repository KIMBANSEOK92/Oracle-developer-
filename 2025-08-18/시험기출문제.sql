...
-- 5. JOIN(INNER, OUTER), 그룹 함수, ROWNUM, 서브쿼리 등 4문제

-- 6. 기본적인 조인부터 조인+서브쿼리+그룹함수 형태까지

-- 7. 테이블은 새로운 테이블 만들어서 제공

-- 8.아래 수준의 문제를 풀 정도면 걱정 안해도 됨
...
FROM CUSTOMER C
INNER JOIN ORDERS S ON C.CUSTID = S.CUSTID
GROUP BY NAME;
...
SELECT COUNT()
FROM BOOK B
INNER JOIN CUSTOMER C  ON B.BOOKID = C.BOOKID
...
SELECT O.ORDERID, O.ORDERDATE, NAME, O.CUSTID 
FROM BOOK B
INNER ORDERS O ON B.BOOKID = O. BOOKID 
INNER CUSTOMER C ON O.CUSTID = C.CUSTID
WHERE TO_CHAR(ORDERDATE, 'DD') = '07' AND BOOKNAME, '이상한미디어'
GROUP BY O.ORDERID, O.ORDERDATE, NAME, O.CUSTID;
...
SELECT B.ORDERID, ROUND(AVG(PRICE))
FROM BOOK B
INNER ORDERS O ON B.BOOKID = O.BOOKID
GROUP BY B.ORDERID

-- PL/SQL 
-- 사용자 정의 함수(원하는대로 쓰는 함수), 프로시저, 트리거(로그관리에 많이 쓰임) 

-- OR REPLACE는 선택
-- 기존에 함수명(MULTI)가 정의 되어 있으면 아래 내용으로 수정하겠다.
-- 선언부(선택), 실해부, 예외처리(선택)
-- 마지막은 /로 닫아야한다.
-- 문자 VARCHAR, 날자 DATE,
-- / 뒤에는 주석을 달면 안된다.
CREATE OR REPLACE FUNCTION MULTI
    (P_VALUE IN NUMBER)  -- FUNCTION은 RETURN타입이 필요하다
RETURN NUMBER            -- EMP.SAL%TYPE <<-- EMP테이블에 SAL을 리턴하겠다는 의미

IS -- 뒤에 오는건 변수선언
BEGIN -- 실행문
    RETURN P_VALUE * 2;
END;
/ 

SELECT MULTI(10)
FROM DUAL;

SELECT SAL, MULTI(SAL) 급여가두배 -- RETURN을 만들어서 빠르게 사용가능
FROM EMP;



-- DATE형 데이터를 받아서 'YYYY-MM-DD HH24:MI:SS' 형태로 리턴
CREATE OR REPLACE FUNCTION DATE_FUNC
    (P_DATE IN DATE)
RETURN VARCHAR2
IS
BEGIN
    RETURN TO_CHAR(P_DATE, 'YYYY-MM-DD HH24:MI:SS');
END;
/

UPDATE TBL_BOARD
SET 
    UDATETIME = SYSDATE
WHERE BOARD IN(1,3,5,7);    
COMMIT ;   


SELECT  
    TITLE,
    DATE_FUNC(UDATETIME),
    TO_CHAR(UDATETIME, 'YYYY-MM-DD HH24:MI:SS')  
FROM TBL_BOARD;


-- DATE_FUNC2(DATE, 'DATETIME') -> 'YYYY-MM-DD HH24:MI:SS';
-- DATE_FUNC2(DATE, 'DATE') ->  'YYYY-MM-DD';
-- DATE_FUNC2(DATE, 'TIME') -> 'HH24:MI:SS';

CREATE OR REPLACE FUNCTION DATE_FUNC2
    (
     P_DATE IN DATE,
     P_KEY IN VARCHAR2
    )
RETURN VARCHAR2       -- RETURN은 파라미터안 2번째에 따라 바뀐다.
IS
BEGIN 
    IF P_KEY = 'DATETIME' THEN
        RETURN TO_CHAR(P_DATE, 'YYYY-MM-DD HH24:MI:SS');
    ELSIF P_KEY = 'DATE' THEN
        RETURN TO_CHAR(P_DATE, 'YYYY-MM-DD');
    ELSIF P_KEY  = 'TIME' THEN
        RETURN TO_CHAR(P_DATE, 'HH24:MI:SS');
    ELSE
        RETURN '키 값 확인바람';
    END IF;
END ;
/


SELECT 
    DATE_FUNC2(UDATETIME, 'DATETIME') || '   ',
    DATE_FUNC2(UDATETIME, 'DATE') || '   ',
    DATE_FUNC2(UDATETIME, 'TIME') 
FROM TBL_BOARD;


-------------------------------------------------
CREATE OR REPLACE FUNCTION SCORE_FUNC
    (P_NUMBER IN NUMBER)
RETURN VARCHAR2
IS
BEGIN
    IF P_NUMBER>100 OR P_NUMBER < 0 THEN
        RETURN '잘못된 숫자';
    ELSIF P_NUMBER >=90  THEN
        RETURN 'A';
    ELSIF P_NUMBER >=80  THEN
        RETURN 'B';
    ELSIF P_NUMBER >=70  THEN
        RETURN 'C';
    ELSE
        RETURN 'D';
    
    END IF;   
    
END;
/


SELECT STU_NAME, ENR_GRADE, SCORE_FUNC(ENR_GRADE)
FROM STUDENT S
INNER JOIN ENROL E ON S.STU_NO = E.STU_NO;